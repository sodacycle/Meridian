...
Window {
    ...
    property real stretchMin: 0.01
    property real stretchMax: 2.0
    property real clipMin:    90.0
    property real clipMax:    99.9
    property int  denoiseMax: 5

    onStretchMinChanged: if (stretchA < stretchMin) stretchA = stretchMin
    onStretchMaxChanged: if (stretchA > stretchMax) stretchA = stretchMax
    onClipMinChanged:    if (stretchP < clipMin) stretchP = clipMin
    onClipMaxChanged:    if (stretchP > clipMax) stretchP = clipMax
    onDenoiseMaxChanged: if (denoiseRadius > denoiseMax) denoiseRadius = denoiseMax

    Settings {
        category: "viewer"
        property alias stretchMin: viewer.stretchMin
        property alias stretchMax: viewer.stretchMax
        property alias clipMin:    viewer.clipMin
        property alias clipMax:    viewer.clipMax
        property alias denoiseMax: viewer.denoiseMax
    }

    property int activeSection: 0
    property var fitsFilePaths: []
    property var displayRows: []
    property var compatiblePaths: matchingPaths(filePath, displayRows)

    onActiveSectionChanged: if (activeSection === 1) refreshFileList()

    function refreshFileList() {
        fitsFilePaths = scanner.listFitsFiles()
    }

    function matchingPaths(path, rows) {
        if (path === "") return []
        if (!rows || rows.length === 0) return sameFolderPaths(path)

        var selected = null
        for (var i = 0; i < rows.length; i++) {
            if ((rows[i]["Path"] || "") === path) { selected = rows[i]; break }
        }
        if (!selected) return sameFolderPaths(path)

        var keys = ["Target", "Filter Used", "Exposure Time s", "Binning", "Gain", "Camera Model", "Telescope"]
        var out = []
        for (var j = 0; j < rows.length; j++) {
            var row = rows[j]
            var same = true
            for (var k = 0; k < keys.length; k++) {
                var a = row[keys[k]] === undefined ? "" : row[keys[k]]
                var b = selected[keys[k]] === undefined ? "" : selected[keys[k]]
                if (String(a) !== String(b)) { same = false; break }
            }
            if (same) {
                var p = row["Path"] || ""
                if (p !== "") out.push(p)
    ...
    function sameFolderPaths(path) {
        var dir = path.substring(0, path.lastIndexOf("/"))
        var all = scanner.listFitsFiles()
        var out = []
        for (var i = 0; i < all.length; i++) {
            if (all[i].substring(0, all[i].lastIndexOf("/")) === dir) out.push(all[i])
        }
        return out.length > 0 ? out : (path !== "" ? [path] : [])
    }
    ...
    Shortcut { sequence: "Left";  onActivated: { if (viewer.activeSection === 0 && viewer.fileIndex > 0) viewer.requestPrevious() } }
    Shortcut { sequence: "Right"; onActivated: { if (viewer.activeSection === 0 && viewer.fileIndex < viewer.fileCount - 1) viewer.requestNext() } }
    ...
    Rectangle {
        ...
        Rectangle { anchors.right: parent.right; width: 1; height: parent.height; color: "#080808" }
        ...
        Column {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    width: navRail.width; height: 62
                    readonly property bool active: viewer.activeSection === index
                    color: active ? "#2c2c2c" : (railHover.hovered ? "#282828" : "transparent")
                    ...
                    Column {
                        ...
                        Canvas {
                            ...
                            width: 24; height: 24
                            anchors.horizontalCenter: parent.horizontalCenter
                            property color iconColor: parent.parent.active ? "#e8e8e8" : "#8a8a8a"
                            ...
                            onPaint: {
                                ...
                                ctx.lineJoin    = "round"
                                ctx.lineCap     = "round"
                                var k = parent.parent.modelData.key
                                if (k === "viewer") {
                                    ctx.strokeRect(3, 5, 18, 14)
                                    ctx.beginPath(); ctx.arc(8, 10, 1.8, 0, Math.PI * 2); ctx.stroke()
                                    ctx.beginPath()
                                    ctx.moveTo(4, 18); ctx.lineTo(10, 12); ctx.lineTo(14, 16)
                                    ctx.lineTo(17, 13); ctx.lineTo(20, 18); ctx.stroke()
                                } else if (k === "files") {
                                    for (var i = 0; i < 3; i++) {
                                        var y = 6 + i * 6
                                        ctx.beginPath(); ctx.arc(5, y, 1.4, 0, Math.PI * 2); ctx.fill()
                                        ctx.beginPath(); ctx.moveTo(9, y); ctx.lineTo(20, y); ctx.stroke()
                                    }
                                } else if (k === "culling") {
                                    ctx.beginPath()
                                    ctx.moveTo(3, 5); ctx.lineTo(21, 5)
                                    ctx.lineTo(14, 13); ctx.lineTo(14, 20); ctx.lineTo(10, 17)
                                    ctx.lineTo(10, 13); ctx.closePath(); ctx.stroke()
                                } else if (k === "stacking") {
                                    for (var j = 2; j >= 0; j--) {
                                        var oy = 5 + j * 4.5
                                        ctx.beginPath()
                                        ctx.moveTo(12, oy); ctx.lineTo(20, oy + 3)
                                        ctx.lineTo(12, oy + 6); ctx.lineTo(4, oy + 3); ctx.closePath(); ctx.stroke()
                                    }
                                } else if (k === "settings") {
                                    for (var r = 0; r < 2; r++) {
                                        var ly = 8 + r * 8
                                        ctx.beginPath(); ctx.moveTo(3, ly); ctx.lineTo(21, ly); ctx.stroke()
                                        var kx = r === 0 ? 8 : 16
                                        ctx.beginPath(); ctx.arc(kx, ly, 2.6, 0, Math.PI * 2)
                                        ctx.fillStyle = "#202020"; ctx.fill()
                                        ctx.fillStyle = iconColor; ctx.stroke()
                                    }
                                }
                            }
                        }
                        ...
                        Text {
                            ...
                            color: parent.parent.active ? "#e8e8e8" : "#8a8a8a"
                            font.pixelSize: 10
                        }
                    }
                    ...
                    MouseArea {
                        ...
                        onClicked: viewer.activeSection = index
                    }
                }
            }
        }
    ...
    Item {
        ...
        anchors.left: navRail.right; anchors.right: parent.right

        Item {
            id: viewerSection
            anchors.fill: parent
            visible: viewer.activeSection === 0
            ...
            Rectangle {
                ...
                Column {
                    ...
                    Slider {
                        width: parent.width
                        from: viewer.stretchMin; to: viewer.stretchMax; stepSize: 0.01
                    ...
                    Slider {
                        width: parent.width
                        from: viewer.clipMin; to: viewer.clipMax; stepSize: 0.1
                    ...
                    Slider {
                        width: parent.width
                        from: 0; to: viewer.denoiseMax; stepSize: 1
                ...
                Column {
                    ...
                    Button {
                        ...
                        text: "Auto-Sort"
                        ToolTip.visible: hovered; ToolTip.delay: 400
                        ToolTip.text: "Open the Culling workspace to compare frames against a control image."
                        onClicked: viewer.activeSection = 2
                    }
            ...
            Rectangle {
                ...
                Column {
                    ...
                    Text {
                        anchors.horizontalCenter: parent.horizontalCenter
                        text: "Pick a file from the Files list, or switch to Culling."
        ...
        CullingView {
            id: cullingSection
            anchors.fill: parent
            visible: viewer.activeSection === 2
            candidatePaths: viewer.compatiblePaths
            stretchA:      viewer.stretchA
        ...
        FileListView {
            anchors.fill: parent
            visible: viewer.activeSection === 1
        ...
        PlaceholderSection {
            anchors.fill: parent
            visible: viewer.activeSection === 3
            title: "Stacking"
            body: "Integration and preprocessing controls are planned for a later phase."
        }

        SettingsView {
            anchors.fill: parent
            visible: viewer.activeSection === 4
            viewer:  viewer
            culling: cullingSection
        }
    }
