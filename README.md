...
- **Detailed Sky View** — a **Detailed View** button (or **Tools → Detailed Sky View… `Ctrl+Shift+P`** on the main window) opens a full 360° interactive sky dome (KStars-style) in its own window: object positions for your location, an imaging-history overlay sizing imaged targets by integration time, horizon/altitude/azimuth grids with cardinal and meridian reference, pan/zoom, and click-to-select objects mirrored two-way with the planner. It opens looking at the horizon in your enabled viewable-area directions (due north when all are enabled), with the level horizon resting in the bottom third of the window, and the dome resizes with the window.
  - **Overnight & date-aware** — the dome shows the **selected night** (Tonight / Tomorrow / any future night, taken live from the planner's night offset) rather than a generic "now" snapshot. It computes that night's **sunset→sunrise** window for the date, renders the star field and object positions at a representative time within it, and clips each object's path to the observing hours. Positions follow the date through the year. When you are actually observing — after sunset at your site on the current night — it switches to the **live current** sky.
  - **RA/Dec grid** — an equatorial graticule of declination parallels and right-ascension meridians, labelled in **RA (hours)** and **Dec (degrees)**. The labels lock to their line but ride the **outer edges** of the window and stay **above the horizon** as you pan/zoom, and both the grid and the compass degree marks between the cardinals subdivide finer the further you zoom in.
...
| Platform | Download |
|---|---|
| Linux x86-64 | [Meridian-x86_64.AppImage Version: 1.0.3a](https://github.com/sodacycle/Meridian/releases/download/1.0.3a/Meridian-x86_64.AppImage) |
